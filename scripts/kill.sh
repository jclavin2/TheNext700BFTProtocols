...
NODENAMES_REGEX="sci|replica|client"
PROCESS_REGEX="server|server_redis|thr_client|thr_client_redis|serverp|clientp|multicast_replica|redis-benchmark"
...
#   echo "Checking line '$node'"
     ...
     ssh -c arcfour -o Compression=no -x -i $HOME/.ssh/id_rsa -o StrictHostKeyChecking=no ${LOGIN_NAME}@$node "${SUDO} pkill '$PROCESS_REGEX' ; sleep 0.2" & 
