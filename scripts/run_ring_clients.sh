...
do
    ...
    then
      ...
      let "CLIENT_COUNTER+=1"  
        ...
        echo -e "./${CLIENT_EXEC} $node $MASTER_NAME $port_quorum $port_backup $port_chain $NUM_BURSTS $NUM_MESSAGES_PER_BURST '$REQUEST_SIZE' ../${CONFIG_QUORUM_CLIENTS} ../${CONFIG_PBFT_CLIENTS} ../${CONFIG_CHAIN_CLIENTS} ../$CONFIG_PRIVATE $INIT_HISTORY_SIZE"
				cd ~/BFT/src/benchmarks;
					
				/tmp/${CLIENT_EXEC} $node $MASTER_NAME $port_quorum $port_backup $port_chain $NUM_BURSTS $NUM_MESSAGES_PER_BURST "$REQUEST_SIZE" ../${CONFIG_QUORUM_CLIENTS} ../${CONFIG_PBFT_CLIENTS} ../${CONFIG_CHAIN_CLIENTS} ../$CONFIG_PRIVATE $INIT_HISTORY_SIZE 2>&1 | tee /tmp/${MYNODE}.${CLIENT_COUNTER}.out || echo ${CLIENT_COUNTER} &
                fi
	fi
