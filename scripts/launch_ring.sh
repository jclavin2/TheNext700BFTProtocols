...
FICTITOUS_EXP_ID=`date +'%Y%m%d-%H%M%S:'`
...
MO_REQUEST_SIZE=${REQUEST_SIZE//[\'\" ]/-}
MASTER_OUTPUT=/tmp/manager.out-${NUM_CLIENTS}-${MO_REQUEST_SIZE}-${REPLY_SIZE}
echo -e "Deploying master on $MASTER_NAME"
echo -e "With user $MASTER_LOGIN_NAME"
...
echo -e "ssh ${MASTER_LOGIN_NAME}@$MASTER_NAME cd BFT/src/benchmarks ; $LIMIT_CMD; ./thr_manager $NUM_CLIENTS $NUM_BURSTS $NUM_MESSAGES_PER_BURST $NUM_MALICIOUS 2>&1 |tee ${MASTER_OUTPUT} & THR_MANAGER_PID=$!"
ssh ${MASTER_LOGIN_NAME}@$MASTER_NAME "cd BFT/src/benchmarks ; $LIMIT_CMD; ./thr_manager $NUM_CLIENTS $NUM_BURSTS $NUM_MESSAGES_PER_BURST $NUM_MALICIOUS" 2>&1 |tee ${MASTER_OUTPUT} &
THR_MANAGER_PID=$!
sleep 2
			...
			then
				...
				${SYSSTAT_START}; \
			    	sudo opcontrol --shutdown; \
			     	sudo ${LD_PRELOAD_LINE} ${ENV_VARS} ./${SERVER_EXEC} ${node} $port_quorum $port_backup $port_chain $REPLY_SIZE $SLEEP_TIME ../$CONFIG_QUORUM ../$CONFIG_PBFT ../$CONFIG_PRIVATE ../$CONFIG_CHAIN 0 0; \
				${SYSSTAT_STOP}; \
...
echo -n ${FICTITOUS_EXP_ID} ${SLEEP_TIME}: >>~/times/experiments.out
perl parse_one_run.pl ${MASTER_OUTPUT} $PROTO |tee -a ~/times/experiments.out

echo -n ${FICTITOUS_EXP_ID} >>~/times/timings.out
ack -h SAMPLES /tmp/server*.out |perl -ple "\$_='${FICTITOUS_EXP_ID}'.\$_;" >> ~/times/timings.out
ack -h TIMINGS /tmp/server*.out |perl -ple "\$_='${FICTITOUS_EXP_ID}'.\$_;" >> ~/times/timings.out
